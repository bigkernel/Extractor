...
class LookupTable {
 public:
  ...
  explicit LookupTable(string_view wanted): table_() {
    int size = sizeof(table_) / sizeof(*table_);
    memset(table_, 0, size);
    for (size_t i = 0; i < wanted.size(); ++ i) {
      table_[Index(wanted[i])] = true;
    }
  }
...
string_view::size_type string_view::find(string_view s, size_type pos) const
  ...
  const char* result =
      memmatch(ptr_ + pos, length_ - pos, s.ptr_, s.length_);
...
const string_view::size_type string_view::npos =
    static_cast<size_type>(-1);
ABSL_STRING_VIEW_SELECTANY
const  string_view::size_type string_view::kMaxSize =
